...
function BasicBlock(filters::Int, downsample::Bool = false, res_top::Bool = false)
    ...
    shortcut = Flux.Chain(Conv((3,3), filters÷2=>filters, pad = (1,1), stride = (2,2)), BatchNorm(filters))
...
function Bottleneck(filters::Int, downsample::Bool = false, res_top::Bool = false)
    ...
    elseif(downsample && res_top)
        return ResidualBlock([filters, filters, filters, 4 * filters], [1,3,1], [0,1,0], [1,1,1], Flux.Chain(Conv((1,1), filters=>4 * filters, pad = (0,0), stride = (1,1)), BatchNorm(4 * filters)))
    else
        shortcut = Flux.Chain(Conv((1,1), 2 * filters=>4 * filters, pad = (0,0), stride = (2,2)), BatchNorm(4 * filters))
